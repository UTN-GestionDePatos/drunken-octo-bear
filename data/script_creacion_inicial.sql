...
CREATE PROCEDURE GESTION_DE_PATOS.ValidarCompraCupon(@id_promocion varchar(30),@fecha datetime,@username varchar(30), @cantidad int, @ret int output)
AS
BEGIN
/*
ret:
	0: pasa las validaciones
...
*/
	...
	(@cantidad - 1 + (select COUNT(*) from GESTION_DE_PATOS.Promociones gc join GESTION_DE_PATOS.Cupones c on c.id_promocion = gc.id_promocion and gc.id_promocion = @id_promocion
	...
	BEGIN
		IF (select stock from GESTION_DE_PATOS.Promociones where id_promocion= @id_promocion)> (@cantidad - 1)
		BEGIN
			...
			IF (select saldo from GESTION_DE_PATOS.Clientes where username=@username) < (@precio_real * @cantidad)
				BEGIN			
					set @ret = 3
					return	
				END	
